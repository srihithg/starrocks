 ...
 *
 * <p>The cache is in memory only and is not written to the edit log or the FE image. An FE that
 * restarts, or a follower promoted to leader that never populated it, answers as it did before this
 * cache existed: "transaction not found". That is the safe direction -- the connector fails loudly
 * and retries -- and the cached status is always copied from the replicated transaction state, never
 * computed independently, so two FEs can disagree about whether they remember a transaction but
 * never about how it ended.
 *
...
class TxnTerminalStateCache {
    ...
    static class Record {
        ...
        // full TransactionState is gone. May be null when the originating transaction had no source
        // type; callers must treat null as "unknown source", never as a match.
    ...
    // This is the ONLY place Records live, so total retention is bounded to exactly the configured
