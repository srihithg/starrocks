...
public class TransactionStateSnapshot {
     ...
     * real id (it is the cache's key), so a cache-sourced snapshot never uses this sentinel. Callers that
     * echo the id back to a client (see the eviction-recovery branches in the HTTP transaction handlers)
     * must skip the field when it is this value so they never emit a bogus {@code "TxnId": -1}.
     */
    ...
    // terminal-state cache record whose transaction had no source type). Callers that gate on the source
    // type (see BypassWriteTransactionHandler) must treat null as "not this source" so an untyped outcome
    // cannot satisfy a request that requires a specific source type.
