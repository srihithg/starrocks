...
public interface TransactionOperationHandler {
     ...
     * real id (the id is the cache's key), so the guard omits the field only for an id-less snapshot --
     * the truly-UNKNOWN outcomes built via the shorter constructors, which the eviction-recovery success
     * branches never reach. The {@code > 0} test (rather than {@code != NO_TXN_ID}) also rejects a zero
