...
public class BypassWriteTransactionHandler implements TransactionOperationHandler {
    ...
    // rejected; the terminal-state cache retains the source type so the same request cannot instead
    // succeed off an untyped cached outcome once the state is gone. Only a genuine cached terminal
    ...
    // source) is rejected, matching the live path.
    @VisibleForTesting
