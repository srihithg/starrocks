...
public class TransactionWithoutChannelHandler implements TransactionOperationHandler {
    ...
    private TransactionResult handlePrepareTransaction(Database db, String label) throws StarRocksException {
        long dbId = db.getId();
        // A terminal (VISIBLE/COMMITTED/ABORTED) status is answered here whether it came from a live
        // TransactionState or, once the state was count-evicted, from the terminal-state cache:
        // getLabelStatus reports both the same way, and the answer is the same either way -- the
        // transaction has already reached its outcome, so a PREPARE retry is answered idempotently
        // rather than redirected. PREPARED (live, still in flight) or UNKNOWN (state lives on the
        // coordinator BE) fall through to a null result -> BE redirect, preserving the ordinary
        // stream-load PREPARE path.
